...
doSubmit = false;
addEvent(document.querySelector('#payment_form'),'submit',doPay);
...
function sdkResponseHandler(status, response) {
    if (status != 200 && status != 201) {
        alert("Por favor, verifica que los datos que ingresaste sean correctos.");
...
function clearOptions() {
    ...
    if (bin.length == 0) {
        ...
        document.querySelector("#label_issuer").style.display = 'none';
        document.querySelector("#issuer").innerHTML = "";
        $('#banner').attr( "src" , "https://imgmp.mlstatic.com/org-img/banners/ar/medios/785X40.jpg" );
...
function guessingPaymentMethod(event) {
    Mercadopago.setPublishableKey("APP_USR-2404bb26-043f-4a4b-8d8f-2610720caf33");
...
function setPaymentMethodInfo(status, response) {
    if (status == 200) {
      console.log(response[0])
        ...
        $('#banner').attr( "src" , response[0].secure_thumbnail );
        $('#banner').removeAttr("width");
        var form = document.querySelector('#payment_form');
        ...
        var cardConfiguration = response[0].settings,
            bin = getBin(),
            amount = document.querySelector('#mp_amount').value;
        ...
        if (issuerMandatory) {
            ...
            document.querySelector("#label_issuer").style.display = 'inline'
